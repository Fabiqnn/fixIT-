...
<form action="{{ route('profile.update_ajax', $user->no_induk) }}" method="POST" id="form-edit" class="space-y-4 font-inter">
    @csrf
    @method('PUT')
        ...
        <div class="p-6 space-y-3">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="no_induk" value="{{ $user->no_induk }}" class="w-full border border-green-200 rounded p-2 outline-none" readonly>
                </div>

                <div>
                    <label class="block mb-1 font-semibold">Password <span class="text-sm text-gray-500">(Kosongkan jika tidak diubah)</span></label>
                    <input type="password" name="password" class="w-full border border-green-200 rounded p-2 outline-none">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nama_lengkap" value="{{ $user->nama_lengkap }}" class="w-full border border-green-200 rounded p-2 outline-none" required>
                    <span id="error-nama_lengkap" class="text-red-500 text-sm block mt-1"></span>
                </div>

                <div>
                    <label class="block mb-1 font-semibold">Level</label>
                    <select name="level_id" id="level_id" class="w-full border border-green-200 rounded p-2 outline-none" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="section-mahasiswa" style="{{ $isMahasiswa ? '' : 'display: none;' }}">
                <div>
                    ...
                    <select name="jurusan_id" class="w-full border border-green-200 rounded p-2 outline-none">
                        ...
                        @foreach ($jurusan as $j)
                            <option value="{{ $j->jurusan_id }}" {{ $user->jurusan_id == $j->jurusan_id ? 'selected' : '' }}>
                                {{ $j->jurusan_nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="prodi_id" class="w-full border border-green-200 rounded p-2 outline-none">
                        ...
                        @foreach ($prodi as $p)
                            <option value="{{ $p->prodi_id }}" {{ $user->prodi_id == $p->prodi_id ? 'selected' : '' }}>
                                {{ $p->prodi_nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="email" name="email" value="{{ $user->email }}" class="w-full border border-green-200 rounded p-2 outline-none">
                    <span id="error-email" class="text-red-500 text-sm block mt-1"></span>
                </div>

                <div>
                    <label class="block mb-1 font-semibold">Nomor Telepon</label>
                    <input type="text" name="nomor_telp" value="{{ $user->nomor_telp }}" class="w-full border border-green-200 rounded p-2 outline-none">
                    <span id="error-nomor_telp" class="text-red-500 text-sm block mt-1"></span>
                </div>
            </div>

            <div class="flex justify-end mt-6">
                <button type="submit" class="bg-green-700 text-white px-6 py-2 rounded hover:bg-green-800 transition cursor-pointer">
                    Simpan Perubahan
                </button>
            </div>
        </div>
...
function initEditValidasi() {
    ...
    $("#form-edit").validate({
        rules: {
            ...
            nomor_telp: { required: true, maxlength: 15 }
        },
